      ...
      if ('interval' == $event['triggerMode']) {
        ...
        if ($intervalStatus === 'wait' || $intervalType === 'na') {
          ...
          $passthroughVars['label'] = $this->translator->trans(
            ...
            [
              '%full%' => $interval . $suffix . ' ' . $this->translator->trans('mautic.campaign.event.intervalunit.customchoice.' . $intervalType) .
              ' of next ' . $this->translator->trans('mautic.campaign.event.intervalunit.customchoice.' . $event['triggerIntervalUnit'])
            ]
          );
        }      
